...
class RMSNorm(nn.Module):
    def __init__(self, dim, eps=1e-8):
        ...
        self.eps = eps
        self.g = nn.Parameter(torch.ones(dim))  # Learnable scale parameter

    def forward(self, x):
        # x: [batch, seq_len, dim]
        # Dimension: RMSNorm operates on embed_dim (3584), not the sequence length, as is typical in transformer layers 
        # where normalization is applied per token.
...
class GroupedQueryAttention(nn.Module):
    ...
    def forward(self, x, use_cache=True, past_seq_len=0):
        ...
        x = self.rms_norm(x)  # [batch_size, seq_len, embed_dim]
        ...
        output = x + attn_output  # [batch_size, seq_len, embed_dim]

        return output

# Test
def test_gqa_with_rms_norm_rope():
