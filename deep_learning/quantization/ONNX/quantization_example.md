...
model `data/mobilenet_v2_float.onnx`.

You can refer to [MobileNetV2 quantization example](./mobilenet.ipynb) to download and convert this ONNX model.

```console
python -m onnxruntime.quantization.preprocess --input data/mobilenet_v2_float.onnx --output data/mobilenet_v2_optimized.onnx
```
...
```console
python run.py --input_model data/mobilenet_v2_optimized.onnx --output_model data/mobilenet_v2_quant.onnx --calibrate_dataset ./calibration_imagenet/
```

The code in [run.py](./run.py) creates an input data reader for the model, uses these input data to run
...
```console
python run_qdq_debug.py --float_model data/mobilenet_v2_optimized.onnx --qdq_model data/mobilenet_v2_quant.onnx --calibrate_dataset ./calibration_imagenet/
```
