...
def train_model(model, train_loader, test_loader, device, num_epochs=20, logger=None):
    ...
    for epoch in range(num_epochs):
        ...
        if logger is not None: 
            logger.info("Epoch: {:03d} Train Loss: {:.3f} Train Acc: {:.3f} Eval Loss: {:.3f} Eval Acc: {:.3f}".format(epoch, train_loss, train_accuracy, eval_loss, eval_accuracy))
        
        # Save model.
        save_model(model=model, model_dir="data", model_filename="resnet18_cifar10_{:03d}.pt".format(epoch))
...
def main():
    ...
    logging.basicConfig(filename="data/cifar.log",
    ...
    # Train model.
    model = train_model(model=model, train_loader=train_loader, test_loader=test_loader, device=cuda_device, num_epochs=100, logger=logger)
    # Save model.
